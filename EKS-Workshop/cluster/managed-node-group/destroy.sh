#!/usr/bin/env bash
# destroy.sh — Delete the EKS managed node group cluster.
# Run from anywhere: the script resolves its own path.
#
# Teardown order:
#   1. Delete EKS cluster (eksctl removes node group, VPC, CloudFormation stacks)
...
echo ""
echo "── Destroy: EKS Managed Node Group Cluster ─────────────────────────────────"
printf "   Cluster : %s\n" "${EKS_CLUSTER_NAME}"
printf "   Region  : %s\n" "${AWS_REGION}"
echo ""
